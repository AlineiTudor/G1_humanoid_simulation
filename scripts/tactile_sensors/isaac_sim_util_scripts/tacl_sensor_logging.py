...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        # ===== RUNTIME DIAGNOSTIC (every 20 steps) =====
        if count % 20 == 0:
            _s = scene["tactile_sensor"]
            # SDF values
            _depth = tactile_data.penetration_depth
            _nf = tactile_data.tactile_normal_force
            _sf = tactile_data.tactile_shear_force
            _depth_sum = _depth[0].sum().item() if _depth is not None else 0
            _nf_sum = _nf[0].abs().sum().item() if _nf is not None else 0
            _sf_sum = _sf[0].abs().sum().item() if _sf is not None else 0
            _depth_max = _depth[0].max().item() if _depth is not None else 0
            _n_contact = (_depth[0] > 0).sum().item() if _depth is not None else 0

            # Positions
            _el_tr = _s._elastomer_body_view.get_transforms()
            _el_pos = _el_tr[0, :3]
            _co_pos_str = "N/A"
            _dist_str = "N/A"
            if _s._contact_object_body_view is not None:
                _co_tr = _s._contact_object_body_view.get_transforms()
                _co_pos = _co_tr[0, :3]
                _co_pos_str = f"({_co_pos[0]:.4f},{_co_pos[1]:.4f},{_co_pos[2]:.4f})"
                _dist_str = f"{(_co_pos - _el_pos).norm().item():.4f}"

            # Tactile points world position
            _tp_w = _s._data.tactile_points_pos_w
            _tp_ctr = _tp_w[0].mean(0) if _tp_w is not None and _tp_w.numel() > 0 else None
            _tp_ctr_str = f"({_tp_ctr[0]:.4f},{_tp_ctr[1]:.4f},{_tp_ctr[2]:.4f})" if _tp_ctr is not None else "N/A"

            print(f"  [step {count:3d}] depth_max={_depth_max:.6f} contacts={_n_contact} "
                  f"normal_F={_nf_sum:.4f} shear_F={_sf_sum:.4f} | "
                  f"elast=({_el_pos[0]:.4f},{_el_pos[1]:.4f},{_el_pos[2]:.4f}) "
                  f"obj={_co_pos_str} dist={_dist_str} pts_ctr={_tp_ctr_str}")
        # ===== END RUNTIME DIAGNOSTIC =====
...
def main():
    ...
    # ===== DIAGNOSTIC: Sensor pipeline state =====
    _sensor = scene["tactile_sensor"]
    print(f"\n{'='*60}")
    print(f"SENSOR PIPELINE DIAGNOSTIC (working example)")
    print(f"{'='*60}")

    # 1. Parent prim (sensor body)
    _parent = _sensor._parent_prims[0]
    print(f"\n[1] Sensor parent prim:")
    print(f"    Path: {_parent.GetPath()}")
    from pxr import UsdPhysics as _UsdPhysics
    print(f"    HasRigidBodyAPI: {_parent.HasAPI(_UsdPhysics.RigidBodyAPI)}")
    print(f"    HasCollisionAPI: {_parent.HasAPI(_UsdPhysics.CollisionAPI)}")

    # 2. Visual mesh found
    if hasattr(_sensor, '_tactile_pos_local') and _sensor._tactile_pos_local is not None:
        _pts = _sensor._tactile_pos_local
    ...
    if hasattr(_sensor, '_contact_object_sdf_view') and _sensor._contact_object_sdf_view is not None:
        print(f"\n[3] Contact object SDF view: CREATED")
    else:
        print(f"\n[3] Contact object SDF view: NONE")

    # 4. Elastomer body view
    if hasattr(_sensor, '_elastomer_body_view') and _sensor._elastomer_body_view is not None:
        _el_tr = _sensor._elastomer_body_view.get_transforms()
        print(f"\n[4] Elastomer body view:")
    ...
    if hasattr(_sensor, '_contact_object_body_view') and _sensor._contact_object_body_view is not None:
        _co_tr = _sensor._contact_object_body_view.get_transforms()
        print(f"\n[5] Contact object body view:")
        print(f"    Pos (env0): ({_co_tr[0,0]:.4f}, {_co_tr[0,1]:.4f}, {_co_tr[0,2]:.4f})")
    else:
        print(f"\n[5] Contact object body view: NONE")

    # 6. Check compliant material
    ...
    _comp_prim = _stage.GetPrimAtPath(_comp_mat_path)
    ...
    if _comp_prim and _comp_prim.IsValid():
        ...
        _physx_mat = _PhysxSchema.PhysxMaterialAPI(_comp_prim)
        if _physx_mat:
    ...
    # 7. List ALL collision prims under sensor parent
    from pxr import Usd as _Usd, UsdGeom as _UsdGeom
    print(f"\n[7] Collision/visual prims under sensor parent:")
    for _prim in _Usd.PrimRange(_parent):
        _has_col = _prim.HasAPI(_UsdPhysics.CollisionAPI)
        _has_mesh_col = _prim.HasAPI(_UsdPhysics.MeshCollisionAPI)
        _is_mesh = _prim.IsA(_UsdGeom.Mesh)
        if _is_mesh:
            _approx = "N/A"
            if _has_mesh_col:
                _approx = _UsdPhysics.MeshCollisionAPI(_prim).GetApproximationAttr().Get()
